...
const store = new Map();
function key(provider, reference) {
 ...
 */
function findRecord(provider, reference) {
...
function insertRecord(record) {
    ...
    if (store.has(k)) {
        throw new Error(`Duplicate idempotency record for ${record.provider}::${record.reference}`);
    }
    store.set(k, record);
}
/**
 ...
 */
function updateRecord(provider, reference, patch) {
